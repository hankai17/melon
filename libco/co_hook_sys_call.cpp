...
static socket_pfn_t g_sys_socket_func 	= (socket_pfn_t)dlsym(RTLD_NEXT,"socket"); //RTLD_NEXT，该参数表示从当前库之后的load进来的动态库中寻找该符号。比如对于socket函数,如果没有其它库也定义了该函数的话，在这里找到的会是glibc中相关socket函数的地址
//即-lco时 会调co库的socket函数 这是RTLD_NEXT保证的  如果co库里想调用原始的socket那么也没问题 这个函数地址已经保存到g_sys_socket_func里了
...
int socket(int domain, int type, int protocol)
{
	...
	int fd = g_sys_socket_func(domain,type,protocol); // 先调用原始的socket函数
	if( fd < 0 )
	{
		return fd;
	}
